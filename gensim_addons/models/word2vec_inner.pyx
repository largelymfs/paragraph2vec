...
ctypedef void (*fast_sentence_ptr) (
    unsigned long long sentence_id,
    const np.uint32_t *word_point, const np.uint8_t *word_code, int codelens[MAX_SENTENCE_LEN],
    REAL_t *neu1,  REAL_t *syn0, REAL_t *syn1, REAL_t *syn0_sen, const int size,
    np.uint32_t indexes[MAX_SENTENCE_LEN], const REAL_t alpha, REAL_t *work,
    int i, int j, int k, int cbow_mean) nogil
ctypedef void (*fast_sentence_test_ptr)(
    unsigned long long sentence_id,
    const np.uint32_t *word_point, const np.uint8_t *word_code, int codelens[MAX_SENTENCE_LEN],
    REAL_t *neu1, REAL_t *syn0, REAL_t *syn1, REAL_t *syn0_sen, const int size,
    np.uint32_t indexes[MAX_SENTENCE_LEN], const REAL_t alpha, REAL_t *work,
...
cdef fast_sentence_test_ptr fast_sentence_test
...
cdef void fast_sentence0(
    ...
    if k>j:
        ...
        size1 = size
    memset(neu1, 0, size1 * cython.sizeof(REAL_t))
    cdef int ptr, now
    ptr = 0
    count = <REAL_t>0.0
    for now in range(size):
        neu1[ptr] = syn0_sen[sentence_id * size + now]
        ptr+=1
    ...
    for m in range(j, k):
        if m == i or codelens[m] == 0:
            ...
            count += ONEF
            for now in range(size):
                neu1[ptr] = syn0[indexes[m] * size + now]
                ptr+=1

            #saxpy(&size, &ONEF, &syn0[indexes[m] * size], &ONE, neu1, &ONE)
    #saxpy(&size, &ONEF, &syn0_sen[sentence_id * size], &ONE, neu1, &ONE)
    count += ONEF
    if cbow_mean and count > (<REAL_t>0.5):
        inv_count = ONEF/count
        sscal(&size, &inv_count, neu1, &ONE)

    memset(work, 0, size1 * cython.sizeof(REAL_t))
    for b in range(codelens[i]):
        row2 = word_point[b] * size
        f = <REAL_t>dsdot(&size, neu1, &ONE, &syn1[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
    ...
    for m in range(j, k):
        if m == i or codelens[m] == 0:
            ...
            for now in range(size):
                syn0[indexes[m] * size + now] += work[ptr]
                ptr+=1
    for now in range(size):
        syn0_sen[sentence_id * size+now] += work[ptr]
        ptr+=1

    #        saxpy(&size, &ONEF, work, &ONE, &syn0[indexes[m] * size], &ONE)
    #saxpy(&size, &ONEF, work, &ONE, &syn0_sen[sentence_id * size], &ONE)
...
cdef void fast_sentence1(
    ...
    for now in range(size):
        ...
        ptr+=1
            ...
            #saxpy(&size, &ONEF, work, &ONEFE, &syn0[indexes[m]*size], &ONE)
    #saxpy(&size, &ONEF, work, &ONE, &syn0_sen[sentence_id * size], &ONE)


cdef void fast_sentence_test0(
    unsigned long long sentence_id,
    const np.uint32_t *word_point, const np.uint8_t *word_code, int codelens[MAX_SENTENCE_LEN],
    REAL_t *neu1, REAL_t *syn0, REAL_t *syn1, REAL_t *syn0_sen, const int size,
    const np.uint32_t indexes[MAX_SENTENCE_LEN], const REAL_t alpha, REAL_t *work,
...
def train_sentence(model, sentence_id, sentence, alpha, _work, _neu1):
    cdef int hs = model.hs
    cdef int negative = model.negative
    cdef int cbow_mean = model.cbow_mean
    cdef unsigned long long _sentence_id = sentence_id

    cdef REAL_t *syn0 = <REAL_t *>(np.PyArray_DATA(model.syn0))
    cdef REAL_t *syn0_sen = <REAL_t *>(np.PyArray_DATA(model.syn0_sen))
    cdef REAL_t *work
    cdef REAL_t *neu1
    cdef REAL_t _alpha = alpha
    cdef int size = model.layer1_size
...
#                k = sentence_len
#            if hs:
#                fast_sentence(_sentence_id, points[i], codes[i], codelens, neu1, syn0, syn1, syn0_sen, size, indexes, _alpha, work, i, j, k, cbow_mean)

    for i in range(window-1, sentence_len):
        if i < 0:
            continue
        j = i - window + 1
        k = i - 1
...
def train_sentence_test(model, sentence_id, sentence, alpha, _work, _neu1):
    cdef int hs = model.hs
    cdef int negative = model.negative
    cdef int cbow_mean = model.cbow_mean
    cdef unsigned long long _sentence_id = sentence_id
...
#                k = sentence_len
#            if hs:
#                fast_sentence_test(_sentence_id, points[i], codes[i], codelens, neu1, syn0, syn1, syn0_sen, size, indexes, _alpha, work, i, j, k, cbow_mean)
        for i in range(window-1, sentence_len):
            if i<0:
                continue
            j = i - window + 1
...
def init():
    ...
    if (abs(d_res - expected) < 0.0001):
        ...
        fast_sentence_test = fast_sentence_test0
        return 0  # double
        ...
        fast_sentence_test = fast_sentence_test1
        return 1  # float
    else:
